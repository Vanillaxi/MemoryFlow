...
func (f *fakeIndexMemoryService) ListForIndex(_ context.Context, limit int, offset int) ([]model.MemoryItem, error) {
...
func (f *fakeDocumentIndexer) Index(_ context.Context, doc IndexDocument) error {
...
func TestIndexerUsesEmbeddingAndVectorStore(t *testing.T) {
	...
	indexer := NewIndexer(&fakeIndexEmbedding{}, store)

	if err := indexer.Index(context.Background(), IndexDocument{MemoryID: 9, Content: "content", MemoryType: "text"}); err != nil {
		t.Fatal(err)
	}
...
func TestIndexerEmbeddingErrorStopsStoreWrites(t *testing.T) {
	store := &fakeIndexVectorStore{}
	indexer := NewIndexer(&fakeIndexEmbedding{err: errors.New("embed failed")}, store)
	if err := indexer.Index(context.Background(), IndexDocument{MemoryID: 9, Content: "content"}); err == nil {
