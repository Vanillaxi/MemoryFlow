...
后端还提供了一个只读 MCP HTTP JSON RPC 原型，方便外部 AI 工具通过本地 token
调用 MemoryFlow 的长期记忆、项目上下文、Web 和 GitHub 只读能力。
...
- MCP HTTP JSON RPC：`POST /mcp/rpc`，只读开放长期记忆、Project Agent / Handoff Summary、Web 和 GitHub 证据查询，返回 `answer`、`used_tools`、`evidence`、`raw_tool_calls`、`pipeline` 和 `project`。
...
MemoryFlow 通过 HTTP JSON RPC 暴露本地 MCP Server，endpoint 是
`POST /mcp/rpc`。外部 AI 工具可以用它调用 MemoryFlow 的核心能力：

- 查询长期记忆。
- 获取项目上下文。
- 生成 Project Handoff Summary。
- 调用只读 Web / GitHub 证据源。
- 返回 `answer`、`evidence`、`used_tools`、`raw_tool_calls`、`pipeline` 和 `project`。

MCP Server 复用现有 dispatcher、`agent.Chat`、`project_pipeline` 和 tools registry。
它是只读接口，默认用于本地调用，不新增 `HandoffTool`，不会写 GitHub、写 Web、写
Todo 或写 Memory，也不会直接暴露本地图片原文件、`config.yaml`、token 或 API key。

### 配置 MCP token

MCP token 不是从第三方平台申请的，而是用户自己本地生成的访问口令。可以用下面的命令
生成：
...
然后写入本地 `backend/configs/config.yaml`：

```yaml
mcp:
  # 本地 MCP HTTP JSON RPC token。
  # 该 token 用于保护 /mcp/rpc，只应该保存在本地 config.yaml 中。
  # token 为空时，MCP 请求应被拒绝。
  token: "mf_local_<your_generated_token>"
```

`configs/config.example.yaml` 只放占位示例；真实的 `configs/config.yaml` 不应该提交到
GitHub。MCP token 只从 YAML 的 `mcp.token` 读取，不支持通过环境变量覆盖。

token 为空时 `/mcp/rpc` 会拒绝请求；token 错误时也会拒绝请求。请求可以把 token 放在
JSON body 的 `token` 字段，也可以使用 `Authorization: Bearer <token>`。推荐使用
Bearer header，避免 token 进入请求 body。
...
- Memory：`query_long_term_memory`、`get_memory_detail`、`aggregate_memory`、`recent_memories`、`timeline`、`project_memories`、`image_memories`
- Project Agent：`project_handoff_summary`、`project_progress_summary`、`project_issue_status`、`project_pr_status`
- Web：`web_fetch`、`web_search`
- GitHub：`get_recent_commits`、`get_recent_issues`、`get_pull_requests`
...
curl -X POST http://localhost:8080/mcp/rpc \
  ...
  -H "Authorization: Bearer mf_local_your_token" \
  -d '{
...
curl -X POST http://localhost:8080/mcp/rpc \
  ...
  -H "Authorization: Bearer mf_local_your_token" \
  -d '{
    "method": "query_long_term_memory",
    "message": "最近关于 MemoryFlow MCP 的记忆",
    "limit": 5
  }' | jq
```

GitHub 只读 evidence 查询：
...
```json
{
  ...
  "evidence": [],
  "raw_tool_calls": [],
  "project": {"id": 1, "name": "MemoryFlow"}
}
```

### 安全边界

- MCP Server 初版只读，只调用白名单方法，不允许任意 tool 名透传。
- `mcp.token` 为空时拒绝请求。
- token 错误时拒绝请求。
- token 不进入 LLM prompt。
- token 不进入 `answer`、`evidence` 或 `raw_tool_calls`。
- 不返回 `config.yaml`、GitHub token、模型 API key 或其他 secret。
- `web_fetch` 仍沿用 Web Tool 的 SSRF 防护：拒绝 `file://`、localhost、回环地址、内网 IP、link-local 和 multicast 地址。
- GitHub 能力只查询 commits、issues 和 PR，不创建 issue、不评论、不 merge、不写仓库。
- 图片记忆只返回 `image_url`、`caption`、`summary` 等 metadata，不直接暴露原始图片文件。
